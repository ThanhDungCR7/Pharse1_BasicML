...
def gen_data_and_vocab():
    os.makedirs('Session4/w2v', exist_ok=True)
    ...
    vocab = [word for word, freq in zip(word_count.keys(), word_count.values())\
        if freq>10]
    vocab.sort()
    with open('Session4/w2v/vocab-raw.txt', 'w') as f:
        f.write('\n'.join(vocab))
    ...
    with open('Session4/w2v/20news-train-raw.txt', 'w') as f:
        f.write('\n'.join(train_data))
    with open('Session4/w2v/20news-test-raw.txt', 'w') as f:
        f.write('\n'.join(test_data))
